...
### Class: `MarketScanner` — 市場掃描器主類
  ...
  - Method: `_detect_2b_signal(self, df, symbol, indicators)` -> Optional[ScanResult] [Calls: _calculate_score, _check_mtf_alignment, _check_confirmed_2b, _check_pre_2b] — 檢測 2B 信號
  ...
  - Method: `scan(self)` -> Tuple[List[ScanResult], MarketSummary] [Calls: layer3_pattern_matching, _generate_market_summary, layer1_liquidity_filter, layer2_momentum_filter, _output_results, layer4_correlation_filter] — 執行完整掃描
  - Method: `_generate_market_summary(self)` -> MarketSummary — 生成市場摘要
  - Method: `_output_results(self)` [Calls: _print_summary, _send_telegram, _output_json, _output_sqlite] — 輸出掃描結果
...
## 📄 File: `trader/bot.py`
**Dependencies:** `sys, os, time, json, signal, logging, logging.handlers, pathlib.(Path), datetime.(datetime, timezone), typing.(Dict, List, Optional), ccxt, pandas, trader.infrastructure.api_client.(BinanceFuturesClient), trader.infrastructure.notifier.(TelegramNotifier), trader.infrastructure.data_provider.(MarketDataProvider), trader.infrastructure.performance_db.(PerformanceDB), trader.indicators.technical.(TechnicalAnalysis, DynamicThresholdManager, MTFConfirmation, MarketFilter), trader.risk.manager.(PrecisionHandler, RiskManager, SignalTierSystem), trader.execution.order_engine.(OrderExecutionEngine), trader.config.(ConfigV6), trader.positions.(PositionManager), trader.persistence.(PositionPersistence), trader.signals.(detect_2b_with_pivots), trader.strategies.base.(Action), argparse`
...
### Class: `TradingBotV6` — V6.0 終極滾倉版交易機器人
  ...
  - Method: `__init__(self)` [Calls: _log_startup, _init_exchange, _restore_positions]
  ...
  - Method: `scan_for_signals(self)` [Calls: fetch_ohlcv, load_scanner_results, _check_total_risk, _check_btc_trend, _execute_trade] — 掃描交易信號
  - Method: `_check_btc_trend(self)` -> Optional[str] — Fetch BTC 1D EMA20/50 trend. Returns 'LONG', 'SHORT', or None on failure.
  - Method: `_refresh_stop_loss(self, pm, new_sl)` [Calls: _place_hard_stop_loss, _cancel_stop_loss_order] — Cancel existing SL order, place new one, update pm.stop_order_id.
  ...
  - Method: `_execute_trade(self, symbol, signal_details, signal_type, tier_multiplier, df_signal)` [Calls: _save_positions, _place_hard_stop_loss, _get_close_side, _futures_create_order, _validate_position_size, _build_log_base, _extract_fill_price, _check_btc_trend] — 執行開倉
  - Method: `monitor_positions(self)` [Calls: _save_positions, _handle_close, fetch_ohlcv, _update_hard_stop_loss, _handle_stage2, _handle_v53_reduce, _build_log_base, _handle_stage3, fetch_ticker] — 監控持倉
  ...
  - Method: `_handle_close(self, pm, current_price)` -> bool [Calls: _save_positions, _calculate_pnl, _futures_close_position, _build_log_base, fetch_ticker] — 處理平倉。
  - Method: `_handle_stage2(self, pm, current_price, df_1h)` [Calls: _refresh_stop_loss, _get_close_side, _futures_create_order, _validate_position_size, _extract_fill_price] — 處理 Stage 2 加倉
  - Method: `_handle_stage3(self, pm, current_price, df_1h)` [Calls: _refresh_stop_loss, _get_close_side, _futures_create_order, _validate_position_size, _extract_fill_price] — 處理 Stage 3 加倉
  - Method: `_handle_v53_reduce(self, pm, pct, label, current_price)` [Calls: _refresh_stop_loss, _calculate_pnl, _futures_close_position, _build_log_base, _extract_fill_price] — 處理 V5.3 減倉
  - Method: `startup_diagnostics(self)` -> bool [Calls: fetch_ohlcv] — 啟動診斷
  - Method: `run(self)` [Calls: _save_positions, _sync_exchange_positions, startup_diagnostics, _adopt_ghost_positions, scan_for_signals, monitor_positions] — 主運行循環
...
### Class: `PositionPersistence` — Position 狀態持久化管理
  ...
  - Method: `clear_positions(self)` -> bool [Calls: save_positions, backup_positions] — 清空 positions.json（慎用！）
...
**Dependencies:** `logging, pandas, typing.(Tuple, Optional, Dict), trader.structure.(StructureAnalysis)`
...
### Class: `RiskManager` — 風險管理類
  ...
  - Method: `get_account_info(self)` -> dict [Calls: get_positions, get_balance] — 獲取完整帳戶資訊
...
**Dependencies:** `logging, requests, typing.(Dict), trader.config.(Config)`
