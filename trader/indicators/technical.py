...
class TechnicalAnalysis:
    ...
    @staticmethod
    # TECH_DEBT: 此函數與 market_scanner.py 的 MarketScanner.calculate_indicators 有重疊邏輯
    ...
    def detect_2B_signal(df: pd.DataFrame) -> Tuple[bool, Optional[Dict]]:
        ...
        if is_bullish_fakeout:
            ...
            signal_details = {
                ...
                'candle_confirmed': current['close'] > current['open']
            }
        ...
        depth_filter = abs(signal_details.get('lowest_point', signal_details.get('highest_point', 0)) -
                          signal_details['stop_level']) < (current['atr'] * 3)
